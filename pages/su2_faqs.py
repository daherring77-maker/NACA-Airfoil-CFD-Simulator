...
with st.expander("Q12: What are the steps involved in compiling SU2 from source on Windows 11?"): 
    ...
    st.write("Step 4 - In the Python environment run command python meson.py setup build --buildtype=release -Dcgns=false -Dtecio=false. Note on Windows 11 disabling cgns will save a lot of trouble and mostly cgns is not required")

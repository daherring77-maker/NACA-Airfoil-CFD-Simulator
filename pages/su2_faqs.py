...
with st.expander("Q12: What are the steps involved in compiling SU2 from source on Windows 11?"): 
    st.write('Step 1 - Download the SU2 Source files and store in a separate directory - note these instructions refer to SU2 version 8.5.0 "Harrier"')
    st.write("Step 2 - Install the Microsoft C++ Build Tools (6 GB) - not the full Visual Studio IDE - which is rather large and not needed for this task")
    st.write("Step 3 - Install meson and ninja in a separate Python environment") 
    st.write("Step 4 - Click the Windows Start Menu and search for x64 Native Tools Command Prompt -  on my system they are under Visual Studio 2026")
    st.write("Navigate to the folder containing the SU2 source code. eg SU2-Code_Source\\SU2-Master") 
    st.write("In this directory run command python meson.py setup build --buildtype=release -Denable-cgns=false -Denable-tecio=false. Note on Windows 11 disabling cgns will save a lot of trouble and mostly cgns is not required")
    st.write("If you don't disable tecio you will get errors LINK : fatal error LNK1104: cannot open file 'libboost_atomic-vc141-mt-x64-1_69.lib. ")
    st.write("The TecIO library (which is Tecplot's output format library) comes bundled with its own Boost 1.69 sources in boost.tar.gz")
    st.write("The easiest solution is to disable TecIO library which is not required if you use Paraview for analysing output")
    st.write('You will probably want to optimize the code using AVX2 or a different instruction set. python meson.py setup build --buildtype=release -Denable-cgns=false -Denable-tecio=false -Dcpp_args="/arch:AVX2" -Dc_args="/arch:AVX2"')
    st.write("Step 5 - Providing meson build works a build subdirectory is created and you are ready to start compiling C++.")
    st.write("Step 6 - Keeping in the same directory enter command ninja -C build" )
    st.write("Step 7 - you are more than likely to hit a number of Warnings - Don't worry about the -march=native warnings! That is a completely harmless warning.")
    ...
    st.write("Step 8 - Fix any compiler errors. In particular you may hit compile errors in Common\\src\\CConfig.cpp on line 1414. The Problem: The function addDoubleArrayOption expects a raw memory pointer (su2double *)." )
    ...
    st.write('Line 1417 needs to change to "addDoubleArrayOption("SPARK_INIT", flamelet_ParsedOptions.spark_init.size(), false, &flamelet_ParsedOptions.spark_init[0]);"')
    st.write("Step 9 - Re-run the Ninja command, it will automatically pick up from where the command failed")
    st.write("Step 10 - Now run ninja -C build and you should find SU2_CFD.exe, SU2_DEF.exe, SU2_SOL.exe, etc. which are ready to use.")
    st.write("Also the time to compile and link from start to finish was only 1 minute on my Windows 11 system - nowhere like the half hour or more quoted in the literature.")
...
with st.expander("Q14: Can pre-compiled versions of SU2 simulate Shock Waves on Aerofoils?"):

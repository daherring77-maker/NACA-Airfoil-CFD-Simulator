...
with st.expander("Q12: What are the steps involved in compiling SU2 from source on Windows 11?"): 
    ...
    st.write("Step 4 - In the Python environment run command python meson.py setup build --buildtype=release -Denable-cgns=false -Denable-tecio=false. Note on Windows 11 disabling cgns will save a lot of trouble and mostly cgns is not required")
    ...
    st.write("Step 9 - Fix any compiler errors. In particular you may hit compile errors in Common\\src\\CConfig.cpp on line 1414. The Problem: The function addDoubleArrayOption expects a raw memory pointer (su2double *)." )
    ...
    st.write("Step 11 - Now run ninja -C build and you should find SU2_CFD.exe, SU2_DEF.exe, SU2_SOL.exe, etc. which are ready to use. Also the exe files are much smaller than the pre-compiled versions. ")

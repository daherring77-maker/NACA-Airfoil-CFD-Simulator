...
with st.sidebar.expander("📐 Geometry & Mesh", expanded=True):
    ...
    if physics_type == "Inviscid (Euler)":
        use_bl = st.checkbox("Generate Boundary Layer Mesh (Optional for Euler)", value=False)
    else:
        st.info("Boundary layer parameters are required for Viscous simulations to capture wall friction.")
        use_bl = True

    # 2. Conditionally show the BL parameters
    if use_bl:
        with st.container():
            st.markdown("#### Near-Wall / Boundary Layer Resolution")
            col1, col2, col3 = st.columns(3)
            with col1:
                first_layer = st.number_input("First Layer Height (m)", value=3e-5, format="%.1e")
            with col2:
                growth_ratio = st.number_input("Growth Ratio", value=1.2, step=0.05, format="%.2f")
            with col3:
    ...
    ext_mesh_size = st.slider(
        "External Mesh Size (m)",
        min_value=0.1,
        value=DEFAULT_EXT_MESH_SIZE,
        step=0.01,
        format="%.2f",
